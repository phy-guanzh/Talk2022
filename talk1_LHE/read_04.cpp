...
struct histos
{
  ...
  histos (TString name, double XS, double lumi = 1) : 
    ...
    makeHisto ("m_mlvlv","m_{lvlv}[GeV]","", 8,0,200) ;
    ...
    makeHisto ("dR_lep_pho","#dR_l1_pho","", 20,0.01,8) ;    
    makeHisto ("dR_lep1_lep2","#dR_l1_l2","", 20,0.01,8) ;    
    /*
...
//      if ( reader.outsideBlock.length() ) std::cout << reader.outsideBlock;
     ...
     // cout << "!!! pass -----> N_mu: " <<v_f_leptons_mu.size ()<< " ---------->N_e: "<< v_f_leptons_mu.size () <<"------!!!" << endl ;
      //cout << "warning, not enough leptons" << v_f_leptons_e.size ()+v_f_leptons_mu.size ()  << endl ;
      auto l1=v_f_leptons_e.at(0);
      //cout<<__LINE__<<endl;
      auto l2=v_f_leptons_mu.at(0);
      auto n1=v_f_neutrinos.at(0);
      auto n2=v_f_neutrinos.at(1);
      auto pho=v_f_photons.at(0);
      auto W1 = buildLP (reader, idx_Ws_e.at (0)-1) ;
      auto _W1= buildLP (reader, idx_Ws_e.at (0)-1) ;    
      auto W2 = buildLP (reader, idx_Ws_mu.at (0)-1) ;
      auto _W2 = buildLP (reader, idx_Ws_mu.at (0)-1) ;
      float weight = reader.hepeup.XWGTUP ;
      #//string test = "FT9_1"; 
      //Double_t weight = reader.hepeup.weight(5);
      //cout << weight <<endl;
      ...
      TLorentzVector v_ll = v_f_leptons_e.at (0) + v_f_leptons_mu.at (0) ;
      ...
      float ptl1 = v_f_leptons_e.at (0).Pt () ;
      float ptl2 = v_f_leptons_mu.at (0).Pt () ;
      float ptg = v_f_photons.at (0).Pt () ;
      if (ptl1 < ptl2) swap (ptl1, ptl2) ;
      if (ptg <20) continue;
      if (ptl1 <25) continue;
      if (ptl2 <20) continue;
      if (l1.DeltaR(pho)<0.25) continue;
      if (l2.DeltaR(pho)<0.25) continue;
...
//      Histos.m_mll->Fill (v_ll.M (), reader.hepeup.XWGTUP) ;
     ...
     // Histos.fill ("etaj2", j2.Eta(), weight) ;
      ...
      Histos.fill ("dR_lep_pho", l1.DeltaR(pho), weight); 
      Histos.fill ("dR_lep_pho", l2.DeltaR(pho), weight); 
      Histos.fill ("dR_lep1_lep2", l1.DeltaR(l2), weight); 
      Histos.fill ("m_ptl1", ptl1, weight) ;
      Histos.fill ("m_ptg", ptg, weight) ;
      Histos.fill ("m_ptg_2", ptg, weight) ;
